...
output "globalenv" {
  value       = local.enabled ? local.globalenv : ""
  description = "Normalized globalenv"
}
...
output "tfrun" {
  value       = local.enabled ? local.tfrun : ""
  description = "Normalized tfrun"
}
...
output "delimiter" {
  ...
  description = "Delimiter between `customer`, `project`, `environment`, `name` and `attributes`"
}
...
# start edit #
locals {
  tags_no_name = { for k, v in local.tags : k => v if k != "Name" }
}
output "tags" {
  value = local.enabled ? (var.name_in_tags ? local.tags : local.tags_no_name) : {}
  # end edit #

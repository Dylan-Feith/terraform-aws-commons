...
# github community tag: 0.12.0


Best Practices:
  - Create a backup plan that backup every night all resources tagged "backup: true" with a retention of 10 days
  - Tag ec2 but not ebs: When a ec2 is backuped, all the attached volumes are backuped. When you will need to restore the ec2, all the parameters are saved, including vpc, security groups, ...
  - You should not backup ec2 in auto-scaling group, because these ec2 should be completely automated and it makes no sense to backup X times the same ec2.
  - Do not configure cold_storage_after, it is only available for efs and we should not need long term storage for efs
  - Incremental backup are not available for all services for example DynamoDB, Aurora or DocumentDB. Every backup is a full backup.": https://docs.aws.amazon.com/aws-backup/latest/devguide/whatisbackup.html

Warning: If you specify multiple criteria in selection_tags, this will be a "OR". So if you set "backup: true" and "type: front" conditions, it will backup every thing that will have either "backup: true" OR "type: front", so an EC2 with "type: front" and "backup: false" will be backuped"!

Examples:

```hcl
module "backup" {
  source = "./terraform-aws-commons/modules/backup/"
...
| <a name="input_enable_continuous_backup"></a> [enable\_continuous\_backup](#input\_enable\_continuous\_backup) | Enable continuous backups for supported resources. | `bool` | `null` | no |
